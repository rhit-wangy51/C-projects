...
/**
 * @brief we read data into superblock_data, then using the fact that we design this struct in a certain way, we do not need 
 * to parse the data. Then we went on extrac useful data in calc_metadata and store it in sturct metadata
 * 
 * @param fd 
 * @param metadata 
 */
...
os_bool_t fetch_inode(os_uint32_t inode_number, int fd,
 ...
 //i.e. we want inode 34, each block group stores 32 of them starting from index 1. we get it is the 2nd in blockgroup 2
  ...
  //i.e. now we need to know which block it is in, note that we have block 0, 1, 2, ....
  os_uint32_t num_inodes_per_block =
    metadata->block_size / sizeof(struct os_inode_t);
  //i.e. now we know there are let's say 4 inode in each block, 
  os_uint32_t inode_block_num =
    offset_within_blockgroup / num_inodes_per_block;// so now we know our inode is the 1st inode info storing block of blockgroup 2
    // which is index 0
  inode_block_num +=
    metadata->bgdt[blockgroup_num].bg_inode_table;// begining block index
  //since we know let's say that the first block of blockgroup 2 is the 40th of the whole system, our inode is located on the 40th 
  //block of the system

  // Step 5.  Calculate the byte offset of this inode within the
  // block that it lives on.
  //i.e. we know it is in the 40th block, but which order?
  os_uint32_t byte_offset_in_block = (offset_within_blockgroup % num_inodes_per_block) * sizeof(struct os_inode_t);
  ...
  lseek(fd, inode_block_num * metadata -> block_size + byte_offset_in_block, SEEK_SET);
...
os_uint32_t file_blockread(struct os_inode_t file_inode, int fd,
  ...
  if ((triple_index != -1) && (ti_blocknum == 0)) {
    ...
    // ti_blocknum is the absolute block number, we move the curser from the start of the system to this block
    ...
    assert((res = read(fd, buffer, metadata->block_size)) ==
           metadata->block_size);// since buffer is a pointer to char that point to the start of the block
    assert(triple_index < (metadata->block_size / 4));
    do_blocknum = *(((os_uint32_t *) buffer) + triple_index);// pointer math to move to the target block number of the next jump
    // buff is an array of indexes
  }
  ...
  if ((!range_in_hole) && (double_index != -1)) {
    ...
    if (do_blocknum == 0) {
      ...
      // fd going to a fix place, not moving that distance, it's absolute position
...
os_bool_t file_read(int fd, int file_inode_num,
  ...
  // if regualr then this is non-zero             not link                                     not directories
  if(((inode -> i_mode & EXT2_S_IFREG) == 0) && ((inode -> i_mode & EXT2_S_IFLNK) == 0) && ((inode -> i_mode & EXT2_S_IFDIR) == 0)){
      return FALSE;
  }
  ...
  *buffer = malloc(malloc_size);
  ...
  for(int i = 0; i < malloc_size / metadata -> block_size; i++){
    read_bytes += file_blockread(*inode, fd, metadata, i, *(buffer) + read_bytes);
  }
...
os_uint32_t scan_dir(unsigned char *directory,
  ...
  while (current_offset < directory_length) {
    ...
    if(strlen(filename) == current_entry_p -> name_len){
      int i = 0;
      while(filename[i] == current_entry_p -> file_name[i]){
        i++;
        if(i == current_entry_p -> name_len){
          // printf("%s to %s\n", filename, current_entry_p -> file_name);
...
os_uint32_t path_to_inode_num(char* path, int fd, struct os_fs_metadata_t *metadata) {
  ...
  os_int32_t current = root;
  //for each / seperated entry in the path
  while(TRUE){
  //1. Use file_read to read the current directory data
  file_read(fd, current, metadata, &inode, &buffer);
  
  //2. Use scan_dir to search for the entry you're looking for
  char* temp = strchr(path_remaining, '/');
  if(temp != NULL){
    *temp = '\0';
  }
  current = scan_dir(buffer, inode.i_size, path_remaining);
  free(buffer);
  path_remaining += strlen(path_remaining) + 1;
  if(temp == NULL){
